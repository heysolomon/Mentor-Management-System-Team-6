...
function Faqs() {
  ...
  return (
    <div className="faqs">
      ...
      {faqs.map((item) => (
        <div className="accordion" key={`${item.id}`} onClick={() => setClickedId(Number(clickedID) === item.id ? 'closed' : item.id)} onKeyUp={() => setClickedId(Number(clickedID) === item.id ? 'closed' : item.id)}>
          ...
          <div className={`accordionBody bg-pri11 rounded-b-lg' ${Number(clickedID) === item.id
            ? '' : 'hidden'}`}
          >
            <p>
      ...
      {faqs2.map((item) => (
        <div className="accordion" key={`1${item.id}`} onClick={() => setClickedId2(Number(clickedID2) === item.id ? 'closed' : item.id)} onKeyUp={() => setClickedId2(Number(clickedID2) === item.id ? 'closed' : item.id)}>
          ...
          <div className={`accordionBody bg-pri11 rounded-b-lg' ${Number(clickedID2) === item.id
            ? '' : 'hidden'}`}
          >
            <p>
              Lorem ipsum dolor sit amet, consectetuer adipiscing elit.
