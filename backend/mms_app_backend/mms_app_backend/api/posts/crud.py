...
def get_posts_crud(db: Session):
    ...
    for post in posts:
        ...
        get_comments = [GetComment(id=c.id, content=c.content, created_at=c.created_at, post_id=c.post_id) for c in
                        comments]
        get_posts.append(GetPost(id=post.id, title=post.title, content=post.content, created_at=post.created_at,
...
def create_comment_crud(db: Session, comment: CreateComment):
    ...
    return GetComment(id=db_comment.id, content=db_comment.content, created_at=db_comment.created_at,
                      post_id=db_comment.post_id)


def get_comments_crud(db: Session):
    comments = db.query(Comment).all()
    return [GetComment(id=comment.id, content=comment.content, created_at=comment.created_at, post_id=comment.post_id)
